...
(def nodes [{:name "server/switchboard" :group 0 :x 250 :y 250 :last-received (now)}
            ...
            {:name "client/scheduler-cmp" :group 1 :x 800 :y 750 :last-received (now)}
...
(def links-vec [{:source :server/ws-cmp :target :server/switchboard :value 1}
                ...
                {:source :client/switchboard :target :client/force-cmp :value 1}
  ...
  (let [x (:x node)
    ...
    [:g {:transform (str "translate(" x "," y ")")}
     ...
     [:text {:dy   "-.5em" :text-anchor :middle :text-rendering "geometricPrecision" :stroke :none
             :fill :black :font-size "11px" :style {:font-weight :bold}} name]
     ...
     [:rect {:x 40 :y 5 :width 10 :height 10 :rx 1 :ry 1 :fill :green
             :style {:opacity (let [opacity (/ (max 0 (- 250 ms-since-tx)) 250)]
                                opacity)}}]
     [:rect {:x -50 :y 5 :width 10 :height 10 :rx 1 :ry 1 :fill :orangered
...
(defn in-handler
  ...
  [app put-fn msg]
  (match msg
         [:firehose/cmp-put  m] (do
                                  (swap! app assoc-in [:nodes-map (:cmp-id m) :last-tx] (now))
                                  (swap! app update-in [:nodes-map (:cmp-id m) :tx-count] #(inc (or % 0))))
         [:cmd/trigger-update] (swap! app assoc :now (now))
         [:firehose/cmp-recv m] (do
                                  (swap! app assoc-in [:nodes-map (:cmp-id m) :last-rx] (now))
                                  (swap! app update-in [:nodes-map (:cmp-id m) :rx-count] #(inc (or % 0))))
