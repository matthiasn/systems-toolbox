...
(defn mouse-view
  ...
  (let [state @app
        ...
        freq (frequencies rtt-times)
        max (apply max rtt-times)
        min (apply min rtt-times)
    ...
    [:div
     [:svg {:width chart-w :height chart-h :style {:background-color :white}
      ...
      [:g
       ...
       (when latency [:text (merge text-default {:y 12 :x 640 }) (str (.toFixed mean 0) "/" min "/" max "/" latency)])
       [:text (merge text-default {:y 12 :x 740 }) "(mean/min/max/last)"]
